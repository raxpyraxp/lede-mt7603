...
INT Set_Power_Offset_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
INT Set_NoForwardingMBCast_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
	...
	switch(cmd & 0x7FFF)
	{
		...
		case RT_OID_WSC_SET_CON_WPS_STOP:
		{
			...
			if(pWscUpnpBandStop)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CON_WPS BAND_STOP_CMD From[%s], isApCli[%d], is2gBand[%d]\n", 
				...
				if (pWscUpnpBandStop->isApCli)
				{
        				...
        				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CON_WPS FROM IOCTL: Stop the ApCli WPS, state [%d]\n", 
								pWpsCtrl->WscState));
				...
				else
				{
					...
					IsAPConfigured = pWpsCtrl->WscConfStatus;
				        ...
				        {
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FROM IOCTL CON_WPS[%d]: Stop the AP Wsc Machine\n", apidx));
        ...
        */
		case OID_AIRPLAY_IE_INSERT:
        {
			...
			if (wrq->u.data.length != 0)
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("os_free_mem fail\n"));
					break;
				}
                ...
                if (Status == NDIS_STATUS_FAILURE)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("os_alloc_mem fail\n"));
					break;
				}
				...
				if (Status == NDIS_STATUS_FAILURE)
				{
				    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set::OID_AIRPLAY_IE_INSERT error!!\n"));
					break;
				}
			...
			{
			    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("wrq->u.data.length == 0, no data need to update\n"));	
			}
        }            
		break;
...
INT	Set_Power_Offset_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT power;
 	...
 	if((pObj->ioctl_if < HW_BEACON_MAX_NUM) && ((power>=0)&&(power<32)))
 	{
 		pAd->ApCfg.MBSSID[pObj->ioctl_if].pwr_offset = power;
   		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Set_Power_Offset_Proc ,set pwr_offset=%d\n ",power));
 		success = TRUE;  		
 	}
 	else
 		success = FALSE;
	...
	IN	RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra%d) Set_NoForwarding_Proc::(IsolateInterStaTraffic=%d)\n",
	...
	IN	RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra%d) Set_NoForwardingMBCast_Proc::(IsolateInterStaMBCast=%d)\n", 
	...
	IN	RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_NoForwardingBTNSSID_Proc::(IsolateInterStaTrafficBTNBSSID=%d)\n",
...
INT Set_PartialScan_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): bScanning = %u \n", __FUNCTION__, pAd->ScanCtrl.PartialScan.bScanning));
	return TRUE;
}
...
#ifdef MAC_REPEATER_SUPPORT
    sprintf(msg+strlen(msg), "BaSnResetCnt         = %ld\n", (ULONG)pAd->RalinkCounters.BaSnResetCnt);
#endif /* MAC_REPEATER_SUPPORT */
	...
	RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I/F(apcli%d) pApCliEntry->TrialCh = %d\n", ifIndex, pApCliEntry->TrialCh));
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I/F(apcli%d) pApCliEntry->TrialCh = %d\n", ifIndex, pApCliEntry->TrialCh));
	...
	IN  RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bBaSnResetDis = %d, Repeater Mode (%s)\n",
...
#ifdef APCLI_SUPPORT		
        pWscControl->WscPINSsidIsSet = TRUE;
#endif
	...
	IN	RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPS is using concurrent WPS now.\n"));
...
INT Set_CutThrough_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():Set CutThroughType as %d\n",
	...
	IN UINT32 ANQPRspLen)
{
	...
	if (ANQPRspLen <= pHSCtrl->MMPDUSize)
	{
		...
		DlListForEach(GASPeerEntry, &pGASCtrl->GASPeerList, GAS_PEER_ENTRY, List)
		{
			...
			{
				IsFound = TRUE;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("GAS RSP DialogToken = %x\n", GASPeerEntry->DialogToken));
