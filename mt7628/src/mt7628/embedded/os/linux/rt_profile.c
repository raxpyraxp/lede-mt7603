...
#define FIRST_AP_PROFILE_PATH          "/etc/wireless/mt7628_ap/mt7628_ap.dat"
...
{
	INT idx = 0;
#if (CONFIG_RT_FIRST_CARD == 7628) && (CONFIG_RT_SECOND_CARD == 7628)
				...
				for(apidx=0;apidx<pAd->ApCfg.BssidNum;apidx++)		
            	{
					...
					if (tr_entry->tx_queue[QID_AC_BE].Head != NULL)
					{
						...
						RTMPDeQueuePacket(pAd, FALSE, QID_AC_BE, wcid, deq_cnt); 
							...
							if (pAd->bDump)
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, (" %s: bss:%u, deq_cnt = %u\n", __FUNCTION__, apidx, deq_cnt));								
	...
	OUT BOOLEAN *DropPacket)
{
	...
	if (IS_GROUP_MAC(pPktHdr)) {
		...
		if ((pInvalidEntry != NULL) || (pOtherBandReptEntry != NULL)) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s, recv broadcast from InvalidRept Entry, drop this packet\n", __func__));
			*DropPacket = TRUE;
		}
	}
}
	...
	IN UCHAR OpMode)
{
	...
	if(RxDMSHandle(pAd, pNetPkt) == NDIS_STATUS_FAILURE)
		return;	
#endif /* CONFIG_STA_SUPPORT */
