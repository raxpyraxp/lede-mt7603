...
/*                pConfig = &Config; */
		...
		case RT_OID_802_11_SET_APSD_PSM:
			...
			else
			{
				...
				/*7636 psm*/
				if (pAd->CommonCfg.bAPSDForcePowerSave != pAd->StaCfg.wdev.Psm)
				{
					RTMP_SET_PSM_BIT(pAd, pAd->CommonCfg.bAPSDForcePowerSave);
				/*7636 psm*/
					RTMPSendNullFrame(pAd, pAd->CommonCfg.TxRate, TRUE, pAd->CommonCfg.bAPSDForcePowerSave ? PWR_SAVE : pAd->StaCfg.wdev.Psm);
				}
    ...
    IN  INT                 cmd)
{
    ...
    switch(cmd)
    {
		...
		case RT_OID_802_11_POWERMANAGEMENTMODE:
			...
			/*7636 psm*/
			if (pAd->StaCfg.wdev.Psm == PSMP_ACTION)
			{
